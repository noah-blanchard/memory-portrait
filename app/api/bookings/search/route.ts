...
export async function GET(req: Request): Promise<NextResponse<ApiResponse<unknown>>> {
  ...
  try {
    ...
    const bookings = data || [];
    const groupedData = {
      pending: bookings.filter((booking: any) => booking.status === 'pending'),
      reviewed: bookings.filter((booking: any) => booking.status === 'reviewed'),
      approved: bookings.filter((booking: any) => booking.status === 'approved'),
      rejected: bookings.filter((booking: any) => booking.status === 'rejected'),
      cancelled: bookings.filter((booking: any) => booking.status === 'cancelled'),
    };
