...
import { Alert, Button, Text, Title } from '@/components/common/i18n';
...
export default function AdminLogin() {
  ...
  const form = useForm({
    ...
    validate: {
      email: (v: string) => (/^\S+@\S+\.\S+$/.test(v) ? null : t('admin_invalid_email')),
      password: (v: string) => (v.length >= 6 ? null : t('admin_password_min')),
    },
  });
