...
export function useUpdateBookingStatus() {
  ...
  return useMutation({
    ...
    onMutate: async ({ requestUid, status }) => {
      ...
      const updateBookingData = (old: BookingGrouped | undefined): BookingGrouped | undefined => {
        if (!old) {return old;}
        ...
        for (const [statusKey, bookings] of Object.entries(old)) {
          const bookingsArray = bookings as BookingRow[];
          const booking = bookingsArray.find((b: BookingRow) => b.request_uid === requestUid);
          if (booking) {
        ...
        newData[sourceStatus] = (newData[sourceStatus] as BookingRow[]).filter(
          (b: BookingRow) => b.request_uid !== requestUid
        );
        
        // Add to target status
        newData[status] = [
          { ...foundBooking, status },
          ...(newData[status] as BookingRow[])
        ];
